...
정수 `number`를 문자열용 지정자 `%s`로 출력하는 실험에서는 `Segmentation fault`가 발생했다. `%s`가 정수 값 `42`를 문자열의 주소로 해석해 접근하려 했기 때문이다. 

### 이게 왜 보안 취약점이 될 수 있는지
포맷 스트링 취약점은 이 실험과 구별된다. 예를 들어 사용자에게 이름을 입력받아 출력할 때 `printf(input)`이라고 작성하면, 사용자가 `민경`을 입력했을 때는 그대로 `민경`이 출력된다. 그러나 `%p`를 입력하면 `printf`는 이를 글자가 아닌 ‘메모리 주소를 출력하라’는 명령으로 해석한다. 사용자가 출력할 내용을 입력했는데 프로그램이 그 내용을 출력 방법으로 사용하게 되는 것이다. 이런 코드에서는 입력에 따라 내부 정보가 노출되거나 프로그램이 비정상 종료될 수 있어 보안 문제가 된다.

이번 실험에서는 포맷 문자열 `%s`를 코드에서 직접 작성했고, 사용자가 이를 바꾸지는 않았다. 따라서 포맷 스트링 취약점을 직접 재현한 것은 아니다. 다만 포맷을 잘못 사용했을 때 `Segmentation fault`가 발생할 수 있음을 확인했다. 사용자 입력을 출력할 때는 `printf("%s", input)`처럼 포맷 문자열을 고정해야 한다. 이렇게 작성하면 사용자가 `%p`를 입력해도 출력 명령으로 실행되지 않고 `%p`라는 글자 그대로 출력된다.
